...
def create_image_lists_with_labels(image_dir, testing_label, validation_label):
  """Builds a list of training images from the file system.

  Analyzes the sub folders in the image directory, splits them into stable
  training, testing, and validation sets using given labels, and returns a
  data structure describing the lists of images for each label and their paths.

  Args:
    image_dir: String path to a folder containing subfolders of images.
    testing_label: A label in the file name indicating a testing sample.
    validation_label: A label in the file name indicating a validation sample.
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      base_name = os.path.basename(file_name)
      if validation_label in base_name:
        validation_images.append(base_name)
      elif testing_label in base_name:
        testing_images.append(base_name)
      else:
        training_images.append(base_name)
    result[label_name] = {
...
def main(_):
  ...
  if FLAGS.testing_label and FLAGS.validation_label:
    image_lists = create_image_lists_with_labels(FLAGS.image_dir, 
                                                 FLAGS.testing_label,
                                                 FLAGS.validation_label)
  else:
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      default=None,
      help="""\
      A label in the image file name indicating the image should be used
      for testing.\
      """
  )
  parser.add_argument(
